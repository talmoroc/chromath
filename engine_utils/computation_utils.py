...
def approximate_frequency_ratio(f1: float, f2: float, d: float = 0.01, divide_by: int = 1) -> tuple[int, int, float]:
    ...
    while r < r_min or r > r_max:
        ...
        if r_init < r:
            a_high, b_high = a, b
            k = math.floor((r0 * b_low - a_low) /
                           (a_high - r0 * b_high))
            a_low, b_low = a_low + k * a_high, b_low + k * b_high
        else:
            a_low, b_low = a, b
            k = math.floor((a_high - r0 * b_high) /
                           (r0 * b_low - a_low))
...
def relative_periodicity(semitones: list[int], reference_index:int = 0, d: float = 0.011, sort: bool = True) -> int:
    ...
    ratios = [
        approximate_frequency_ratio(reference_freq, 2 ** ((s + 12*(i < reference_index)) / 12), d, divide_by=(i < reference_index)+1) for i, s in enumerate(semitones)
    ]
